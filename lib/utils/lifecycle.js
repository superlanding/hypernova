...
function raceTo(promise, ms, msg) {
  ...
  return _promise2['default'].race([promise, new _promise2['default'](function (resolve) {
...
function runAppLifecycle(lifecycle, plugins, config, error) {
  ...
  try {
    var promise = _promise2['default'].all(plugins.filter(hasMethod(lifecycle)).map(function (plugin) {
...
function processJobsConcurrently(jobs, plugins, manager) {
  return _promise2['default'].all(Object.keys(jobs).map(function (token) {
    return processJob(token, plugins, manager);
  }));
}
